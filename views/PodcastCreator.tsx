...
import { Mic, Users, Music, Plus, Edit3, PlayCircle } from 'lucide-react';
...
export const PodcastCreatorView: React.FC<PodcastCreatorProps> = ({ onBack }) => {
  return (
    <div className="flex flex-col h-full bg-petrol animate-in slide-in-from-right duration-300">
      <div className="px-6 py-6 border-b border-white/5 flex justify-between items-center bg-petrol z-10">
        ...
        <button onClick={onBack} className="text-sm text-neon font-medium">Sair</button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 pb-24 space-y-8">
        ...
        <section>
            ...
            <input type="text" defaultValue="Episódio #01 - O Futuro da AI" className="w-full bg-graphite border border-white/10 rounded-xl p-3 text-white focus:border-rose-500 focus:outline-none" />
        </section>
        ...
        <div className="flex gap-3">
            ...
            <button className="flex-1 bg-rose-600 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-rose-500 transition-colors shadow-lg shadow-rose-500/20">
